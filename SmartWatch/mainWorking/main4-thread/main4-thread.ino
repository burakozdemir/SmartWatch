...
void setup() {
  ...
  sdCardSet();
  delay(2000); 
  //pthread_create(&threads[0], NULL, ivmeThread, (void *)1);
  //pthread_create(&threads[1], NULL, nabizThread, (void *)1);
}
...
void sendHttp(String val){
   ...
   String temp="data=";
   temp+=val; 
   int httpResponseCode = http.POST(returnValues(0));   //Send the actual POST request
...
void loop() {
  ...
  if(startTime==0) startTime=millis();
      ...
      if(wifiFlag==0) allDataToSD();
      
      if(fark>mainDelay && wifiFlag!=1){
        startTime=0;
        endTime=0;
        wifiFlag=1;
        pthread_create(&threads[0], NULL, wifiThread, (void *)1);
